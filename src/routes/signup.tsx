...
function SignUpPage() {
  ...
  const handlePasswordSignUp = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (password !== confirmPassword) {
      setError(m.auth_error_passwords_do_not_match());
      return;
    }
    ...
    try {
      ...
      const emailRedirectTo = `${import.meta.env.VITE_BASE_URL || window.location.origin}/auth/callback`;
      ...
      if (authError) {
        setError(authError.message || m.signup_error_generic());
        return;
      }
      ...
      setNotice(m.signup_check_email());
    } catch (err) {
      console.error("Auth error:", err);
      setError(m.signup_error_unexpected());
    } finally {
      setIsLoading(false);
    }
  };
